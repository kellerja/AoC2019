...
class IO:
    def __init__(self, pipes=None, listeners=None, wait_input=input):
        self.stream = []
        self.inputs = []
        self.pipes = pipes
...
class OutputCollector:
    ...
    def __init__(self, output_method=Method.COUNT, output_until=0, output_count=1, callback=None):
    ...
    def listen(self, value):
        if self.output_method == OutputCollector.Method.COUNT:
            self.output_batch.append(value)
            if len(self.output_batch) >= self.output_count:
                if self.callback is not None:
                    self.callback(self.output_batch.copy())
                self.output_batch = []
        elif self.output_method == OutputCollector.Method.END_INT_EXCLUDE:
            if value == self.output_until:
                if self.callback is not None:
                    self.callback(self.output_batch.copy())
                self.output_batch = []
            else:
    ...
    def attach(self, process):
        if process.io.listeners is None:
            process.io.listeners = [self.listen]
        else:
            process.io.listeners.append(self.listen)
